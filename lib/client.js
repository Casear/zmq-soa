  ...
  Client = (function(_super) {
    ...
    Client.prototype.onMsg = function() {
      ...
      if (msg instanceof messages.client.ReadyMessage) {
        ...
        return this.onClientTimeoutMessage(msg);
    ...
    Client.prototype.onClientTimeoutMessage = function(msg) {
      var hex;
      if (msg.mapping) {
        ...
        if (msg.mapping && this.callback[hex]) {
          ...
          this.callback[hex]('timeout', msg.data);
    ...
    Client.prototype.onWorkerMessage = function(msg) {
      ...
      if (this.workerCallback) {
        ...
        logger.debug(msg);
        cb = function(returnMsg, encrypt) {
          var r;
          if (msg.mapping) {
            r = new messages.worker.ResponseMessage(msg.service, new Buffer(returnMsg), null, msg.mapping);
            if (!encrypt) {
              return this.SendWithOutEncrypt(r);
            } else {
              return this.SendWithEncrypt(r);
            }
          }
        };
    ...
    Client.prototype.onAuth = function(msg) {
      ...
      if (msg.data) {
        ...
        if (!this.isAuth) {
          this.isAuth = true;
          return this.emit('authenticate', true);
        }
      } else {
        return this.emit('authenticate', false);
      }
    };
    ...
    Client.prototype.sendBService = function(msg, callback, timeout) {
      var buf, hex, num;
      if (this.connected) {
        if (this.isAuth) {
          if (callback) {
            ...
            this.callbackTimeout[hex] = setTimeout((function() {
            ...
            this.SendWithEncrypt(new messages.client.BServiceMessage(msg, null, buf, timeout));
          } else {
            this.SendWithEncrypt(new messages.client.BServiceMessage(msg, null, null, timeout));
          }
          return logger.debug('client : sent bservice ' + msg);
        } else {
    ...
    Client.prototype.send = function(service, msg, encrypt, callback, timeout) {
      var buf, hex, num;
      logger.debug('client : sending ' + msg + ' connected:' + this.connected + ' auth:' + this.isAuth);
      if (this.connected) {
        if (this.isAuth) {
          if (callback) {
            buf = new Buffer((function() {
            ...
            if (encrypt) {
              this.SendWithEncrypt(new messages.client.RequestMessage(service, msg, null, buf, timeout));
            } else {
              this.SendWithOutEncrypt(new messages.client.RequestMessage(service, msg, null, buf, timeout));
            }
          } else {
            if (encrypt) {
              this.SendWithEncrypt(new messages.client.RequestMessage(service, msg, null, null, timeout));
            } else {
              this.SendWithOutEncrypt(new messages.client.RequestMessage(service, msg, null, null, timeout));
            }
          }
          return logger.debug('client : sent ' + msg);
        } else {
          logger.error('client auth failed ');
          if (callback) {
            return callback('auth failed');
    ...
    Client.prototype.SendWithOutEncrypt = function(msg) {
      return this.socket.send(msg.toFrames());
    };
