    ...
    Client.prototype.send = function(service, msg, callback, timeout) {
      ...
      if (this.connected) {
        if (callback) {
          ...
          this.socket.send(new messages.client.RequestMessage(service, JSON.stringify(msg), null, buf).toFrames());
        } else {
          this.socket.send(new messages.client.RequestNoRMessage(service, JSON.stringify(msg), null).toFrames());
        }
