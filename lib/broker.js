(function() {
  var Broker, EventEmitter, authClientFunc, authWorkerFunc, brokerServiceFunc, crypto, fs, heartbeatTime, keySize, logger, messages, redis, rsa, ursa, zmq, _,
  ...
  Broker = (function(_super) {
    ...
    function Broker(endpoint, options, log, brokerService) {
      var keyContent;
      ...
      setTimeout(this.executeQueue.bind(this), 1);
      setTimeout(this.pubQueue.bind(this), 1);
    }
    ...
    Broker.prototype.pubQueue = function() {
      ...
      return setTimeout(this.pubQueue.bind(this), 1);
    };
    ...
    Broker.prototype.executeQueue = function() {
      ...
      if (this.queue.length > 0) {
        ...
        if (message instanceof messages.client.RequestMessage) {
          ...
          if (message.mapping) {
            this.emit('service', worklabel, message);
          } else {
            this.emit('service', worklabel, message, brokerServiceFunc.bind(this));
          }
        }
      }
      return setTimeout(this.executeQueue.bind(this), 1);
    };
    ...
    Broker.prototype.Timeout = function(worklabel, time) {
      return setTimeout((function() {
        ...
        if (this.mapping[worklabel]) {
          ...
          this.SendWithEncrypt(new messages.client.TimeoutMessage(clientEnvelope, mapEnvelope));
    ...
    Broker.prototype.onMessage = function(envelope) {
      var d, data, decipher, decrypted, e, encrypt, message, s;
      ...
      if (message) {
        if (message instanceof messages.client.Message) {
          ...
          if (message instanceof messages.worker.HeartbeatMessage) {
            ...
            return this.onWorkerResponse(message, envelope, encrypt);
          } else if (message instanceof messages.worker.DisconnectMessage) {
            logger.debug('broker: on worker Disconnect');
            return this.onWorkerDisconnect(message, envelope);
    ...
    Broker.prototype.onClientHeartBeat = function(envelope) {
      ...
      if (this.clients[e]) {
        ...
        this.clients[e].checkHeartbeat = setTimeout((function() {
          this.emit('disconnect', this.clients[e]);
          return logger.info('Client ' + e + ' Disconnect');
        }).bind(this), heartbeatTime);
      ...
      if (this.workers[e]) {
        ...
        this.workers[e].checkHeartbeat = setTimeout((function() {
          ...
          if (this.workers[e]) {
            ...
            this.emit('disconnect', this.workers[e]);
            logger.info('Service ' + this.workers[e].service + ' Worker ' + e + ' Disconnect');
    ...
    Broker.prototype.onClientRequest = function(message, encrypt) {
      var clientEnvelope, mapEnvelope;
      if (this.services.hasOwnProperty(message.service.toString())) {
        ...
        if (encrypt) {
          ...
          this.SendWithOutEncrypt(new messages.client.ResponseMessage(message.service, JSON.stringify({
            result: 0,
            err: '服務不存在'
          }), clientEnvelope, mapEnvelope));
        }
    ...
    Broker.prototype.onServiceRequest = function(message) {
      this.queue.push(message);
      logger.debug(message);
      return logger.info(message.envelope.toString('hex'), " to Broker");
    };

    Broker.prototype.onWorkerResponse = function(message, envelope, encrypt) {
      ...
      if (message.mapping) {
        ...
        if (this.mapping[workerlabel]) {
          ...
          if (encrypt) {
            ...
            this.SendWithOutEncrypt(new messages.client.ResponseMessage(message.service, message.data, clientEnvelope, mapEnvelope));
          }
    ...
    Broker.prototype.onWorkerDisconnect = function(message, envelope) {
      var e;
      e = envelope.toString('hex');
      if (this.workers[e]) {
        logger.error('=====disconnect message========');
        return logger.error(this.workers[e]);
      }
    };
    ...
    Broker.prototype.SendWithOutEncrypt = function(msg) {
      var e, ex;
      try {
        e = msg.envelope.toString('hex');
        if (this.workers[e] && this.workers[e].isReady) {
          this.socket.send(msg.toFrames());
          return logger.debug('Sent');
        } else if (this.clients[e] && this.clients[e].isReady) {
          this.socket.send(msg.toFrames());
          return logger.debug('Sent');
    ...
    Broker.prototype.Pub = function(service, msg) {
      var worker, ws, _i, _len, _results;
      if (this.services[service]) {
        ws = this.services[service].waiting;
        _results = [];
        for (_i = 0, _len = ws.length; _i < _len; _i++) {
          worker = ws[_i];
          _results.push(this.pqueue.push({
            service: service,
            worker: worker,
            data: msg
          }));
        ...
        return logger.info("service:" + service + " isnt exist");
      }
    };
  ...
  authClientFunc = function(result, envelope, data, auth) {
    var e;
    if (result) {
      ...
      if (this.clients[e]) {
        ...
        this.clients[e].auth = auth;
        this.SendWithEncrypt(new messages.client.AuthMessage(data, envelope));
        return this.emit('connect', this.clients[e]);
      } else {
  ...
  authWorkerFunc = function(result, envelope, service, data, auth) {
    var e;
    if (result) {
      ...
      if (this.workers[e]) {
        ...
        this.emit('connect', this.workers[e]);
      } else {
  ...
  brokerServiceFunc = function(workerlabel, d) {
    var clientEnvelope, mapEnvelope;
    if (this.mapping[workerlabel]) {
      clientEnvelope = this.mapping[workerlabel].envelope;
      mapEnvelope = this.mapping[workerlabel].mapping;
      delete this.mapping[workerlabel];
      this.SendWithEncrypt(new messages.client.ResponseMessage(null, d, clientEnvelope, mapEnvelope));
      return logger.info("Broker to ", message.service.toString(), ' return');
    }
  };
